...
\end{tikzpicture} \\
\end{tabular}
\caption{Example of unique Voronois tiles (from Figure \ref{fig_finiteSectionsVoronoiExample}) with their domains from the quasicrystal for $\beta_8 = 1+\sqrt{2}$, $n=4$ and window $\Omega = \left[-\frac{1+\sqrt{2}}{6},\frac{1+\sqrt{2}}{6}\right)$. Voronoi tiles are represented by the thick line. }%]$
...
\node [right] at ($(0.55000,-0.34315)$) {$10-4\beta_8$};

\node [right] at ($(0.55000,0)+2*(trans01)$) {$\frac{1}{\beta_8}$};
\node [right] at ($(0.55000,0)+2*(trans03)$) {$2\beta_8-4$};
\node [right] at ($(0.55000,0)+2*(trans04)$) {$1$};

\end{tikzpicture} 
\caption{The section $\Phi(V)$ for Voronoi tile in Figure \ref{fig_calculatingPeriodExample01} for window sizes in $[10-4\beta_8, 1]$. Horizontal slices represent the window of each size and the rhombus show the increasing and decreasing size of the section $\Phi(V)$. }
